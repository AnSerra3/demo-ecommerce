...
public class DBinitConfig {
    @Bean
    CommandLineRunner commandLineRunner(CartRepository cartRepository, CustomerRepository customerRepository, ProductRepository productRepository) {
        return args -> {
            log.info("Inserting demo carts and products in the database");
            Customer alice = customerRepository.save(Customer.builder().name("Alice").email("alice@gmail.com").build());
            Customer bob = customerRepository.save(Customer.builder().name("Bob").email("bob@gmail.com").build());
